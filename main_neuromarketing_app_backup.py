...
# Enhanced Tab 1: Advanced Text Analysis with Visualizations
with tab1:
    st.header("🧠 Advanced Text Analysis")
    st.markdown("**Comprehensive sentiment analysis with advanced visualizations**")
    ...
    with col1:
        user_input = st.text_area(
            ...
            placeholder="Type or paste your text here for advanced sentiment analysis...",
            height=150,
            key="tab1_input"
        )
    ...
    with col2:
        ...
        real_time = st.checkbox("Real-time Updates", value=False)
    
    if user_input:
        try:
            # Perform sentiment analysis (using your existing logic)
            ...
            # Calculate confidence (mock calculation - you can enhance this)
            confidence = min(0.95, abs(sentiment_score) + 0.5)
            
            # Create emotion data (enhanced analysis)
            emotions = {
                'Joy': max(0, sentiment_score * 0.8 + np.random.normal(0, 0.1)),
                'Trust': max(0, sentiment_score * 0.6 + np.random.normal(0, 0.1)),
                'Anticipation': max(0, sentiment_score * 0.5 + np.random.normal(0, 0.1)),
                'Surprise': max(0, abs(sentiment_score) * 0.3 + np.random.normal(0, 0.1)),
                'Fear': max(0, -sentiment_score * 0.4 + np.random.normal(0, 0.1)),
                'Sadness': max(0, -sentiment_score * 0.6 + np.random.normal(0, 0.1)),
                'Disgust': max(0, -sentiment_score * 0.3 + np.random.normal(0, 0.1)),
                'Anger': max(0, -sentiment_score * 0.5 + np.random.normal(0, 0.1))
            }
            
            # Normalize emotions to 0-1 range
            max_emotion = max(emotions.values()) if max(emotions.values()) > 0 else 1
            emotions = {k: min(1, max(0, v/max_emotion)) for k, v in emotions.items()}
            
            # Display Results with Visualizations
            st.markdown("---")
            st.subheader("📊 Analysis Results")
            
            # Row 1: Sentiment Gauge and Basic Metrics
            col1, col2, col3 = st.columns([2, 1, 1])
            
            with col1:
                st.markdown("**Sentiment Score Gauge:**")
                gauge_fig = sentiment_viz.create_sentiment_gauge(sentiment_score, sentiment_label, confidence)
            ...
            with col3:
                st.metric(
                    label="Text Length",
                    value=f"{len(user_input)} chars",
                    delta=f"{len(user_input.split())} words"
                )
                
                st.metric(
                    label="Complexity",
                    value="Medium",  # You can enhance this calculation
                    delta="Readable"
                )
            
            # Row 2: Emotion Radar Chart
            if enable_advanced:
                st.markdown("**Emotion Analysis Radar:**")
                radar_fig = sentiment_viz.create_emotion_radar_chart(emotions)
            ...
            if show_confidence:
                ...
                with col2:
                    st.markdown("**Real-time Sentiment Flow:**")
                    # Create a simple line chart for real-time flow
                    import pandas as pd
                    
                    # Generate mock real-time data
                    time_points = list(range(10))
                    sentiment_flow = [sentiment_score + np.random.normal(0, 0.1) for _ in time_points]
                    ...
                    st.line_chart(flow_df.set_index('Time'))
            ...
            with analysis_col1:
                ...
                st.write(f"• **Primary Emotion**: {max(emotions, key=emotions.get).title()}")
                st.write(f"• **Sentiment Strength**: {abs(sentiment_score):.2f}")
                st.write(f"• **Emotional Balance**: {'Positive' if sentiment_score > 0 else 'Negative' if sentiment_score < 0 else 'Neutral'}")
                st.write(f"• **Confidence Level**: {confidence:.1%}")
            
            with analysis_col2:
                st.markdown("**Recommendations:**")
                if sentiment_score > 0.5:
                    st.success("✅ Strong positive sentiment - Great for marketing!")
                elif sentiment_score > 0.1:
                    st.info("📈 Mild positive sentiment - Consider enhancement")
                elif sentiment_score < -0.5:
                    st.error("⚠️ Strong negative sentiment - Needs revision")
                elif sentiment_score < -0.1:
                    st.warning("📉 Mild negative sentiment - Review recommended")
                else:
                    st.info("⚖️ Neutral sentiment - Consider adding emotional appeal")
        ...
        st.info("👆 Enter text above to see advanced sentiment analysis with visualizations")
        
        # Show example visualizations with sample data
        st.markdown("**Preview of Analysis Capabilities:**")
        
        col1, col2 = st.columns(2)
        
        with col1:
            sample_gauge = sentiment_viz.create_sentiment_gauge(0.6, "positive", 0.85)
            st.plotly_chart(sample_gauge, use_container_width=True)
        
        with col2:
            sample_emotions = {
                'Joy': 0.7, 'Trust': 0.5, 'Anticipation': 0.6, 'Surprise': 0.3,
                'Fear': 0.2, 'Sadness': 0.1, 'Disgust': 0.1, 'Anger': 0.1
            }
            sample_radar = sentiment_viz.create_emotion_radar_chart(sample_emotions)
